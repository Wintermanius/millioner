...
export const AnswersBox: FC<AnswersBoxProps> = ({ questionNumber, correctAnswer, onChangeAnswers }) => {
  ...
  return (
    <Container>
      {answers1 && answers1.map((item, index) => <Answer onClick={() => handleCheckAnswer(item)} key={index}>{item}</Answer>)}
    </Container>
  )
}
